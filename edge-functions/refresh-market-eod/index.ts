...
const OPERATION = "REFRESH_EOD" as const
const MAX_SECURITIES = 25
const LOOKBACK_DAYS = 10
const LEASE_SECONDS = 180
const SUCCESS_COOLDOWN_SECONDS = 10
const FAILURE_COOLDOWN_SECONDS = 2
...
function required(name: string) {
  ...
  if (!value) throw new PublicError("SERVER_CONFIG_MISSING", "Incremental EOD service is not configured.")
  return value
}
...
function decodeBase32(value: string) {
  ...
  for (const character of normalized) {
    ...
    if (index < 0) throw new PublicError("TOTP_CONFIG_INVALID", "Incremental EOD service is not configured.")
...
async function angelJson<T>(response: Response): Promise<AngelResponse<T>> {
  ...
  try {
    ...
    // Angel One can return an empty throttling response.
  }
  if (!response.ok || body?.status !== true) {
    const code = typeof body?.errorcode === "string" && body.errorcode ? body.errorcode : `HTTP_${response.status}`
    const status = response.status === 429 || response.status === 403 ? 429 : 502
    throw new PublicError(code, "Angel One incremental EOD request failed.", status)
  }
  return body
}
...
async function login(value: AngelConfig) {
  const response = await fetch("https://apiconnect.angelone.in/rest/auth/angelbroking/user/v1/loginByPassword", {
    ...
    body: JSON.stringify({ clientcode: value.clientCode, password: value.pin, totp: await totp(value.totpSecret) }),
  })
  const body = await angelJson<{ readonly jwtToken?: string }>(response)
  const jwt = body.data?.jwtToken
  if (!jwt) throw new PublicError("ANGEL_LOGIN_FAILED", "Angel One incremental EOD request failed.", 502)
  return jwt
}
...
function volume(value: unknown) {
  ...
  const numeric = Number(decimal(value, "volume"))
...
async function acquireLease(admin: ReturnType<typeof createClient>, portfolioId: string, holder: string) {
  ...
  if (error) throw new PublicError("LEASE_ACQUIRE_FAILED", "Incremental EOD operation could not be started.")
  const row = Array.isArray(data) ? data[0] as { acquired?: unknown; retry_after?: unknown } | undefined : undefined
  if (row?.acquired !== true) {
    const retryAfter = typeof row?.retry_after === "number" ? row.retry_after : null
    throw new PublicError(
      "MARKET_DATA_RATE_LIMITED",
      retryAfter ? `Incremental EOD refresh is cooling down. Retry in about ${retryAfter}s.` : "Incremental EOD refresh is already running or in cooldown.",
      429,
    )
  }
}
...
async function releaseLease(
  ...
  if (error) console.error("[refresh-market-eod] lease release failed")
}
...
async function fetchCandles(value: AngelConfig, jwt: string, mapping: MappingRow, fromDate: string, toDate: string) {
  ...
  return (body.data ?? []).map((row) => {
    if (!Array.isArray(row) || row.length < 5) throw new PublicError("INVALID_CANDLE", "Angel One returned an invalid candle row.", 502)
    return {
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { fromDate, toDate } = incrementalRange()
    ...
    if (!verified.length) {
      succeeded = true
      return json(200, { runId, requested: securityIds.length, fetchedSecurities: 0, unresolved, failed: 0, candlesUpserted: 0, fromDate, toDate })
    }
      ...
      try {
        ...
        if (rows.length) {
          const { error: historyError } = await admin
            ...
            .upsert(rows, { onConflict: "security_id,provider_code,trade_date" })
        ...
        console.error(`[refresh-market-eod] security fetch failed code=${code}`)
        if (error instanceof PublicError && (error.status === 429 || error.status === 401)) throw error
      }
      if (index < verified.length - 1) await new Promise((resolve) => setTimeout(resolve, REQUEST_DELAY_MS))
    }
    ...
    return json(200, { runId, requested: securityIds.length, fetchedSecurities, unresolved, failed, candlesUpserted, fromDate, toDate })
  } catch (error) {
    const safe = error instanceof PublicError
      ? error
      : new PublicError("EOD_REFRESH_INTERNAL_ERROR", "Incremental EOD refresh failed.")
    console.error(`[refresh-market-eod] code=${safe.code}`)

    if (admin && runId) {
      await admin
        .from("market_data_refresh_runs")
        .update({ status: "FAILED", completed_at: new Date().toISOString(), error_summary: safe.code })
        .eq("id", runId)
    }
    ...
    if (admin && portfolioId && leaseHolder) {
      await releaseLease(admin, portfolioId, leaseHolder, succeeded ? SUCCESS_COOLDOWN_SECONDS : FAILURE_COOLDOWN_SECONDS)
    }
  }
})
