...
export const Route = createFileRoute("/_app/import/$batchId")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Confirm securities and source facts before committing an import." },
      { property: "og:title", content: "Review import — PortfolioAI" },
      { property: "og:description", content: "Confirm securities and source facts before committing an import." },
    ],
  }),
  component: BatchPage,
});
...
const ALLOW_MISSING_BROKER_ACCOUNT = import.meta.env["VITE_M11_NULL_BROKER_COMMIT"] === "true";
...
function BatchPage() {
  ...
  const derived = useMemo(() => {
    return rows.map((row) => {
      const choice = choices[row.id] ?? { securityId: null, brokerAccountId: null, excluded: false };
      ...
      const verdict = evaluateRow(
        ...
        },
        {
          ...
          allowMissingBrokerAccount: ALLOW_MISSING_BROKER_ACCOUNT,
        },
      );
  ...
  const incompleteReadyCount = includedRows.filter((item) => item.verdict.ready && item.verdict.issues.length > 0).length;
  ...
  const brokerGroups = useMemo(() => {
    ...
    for (const item of derived) {
      ...
      const entry = map.get(key) ?? { label: raw === "" ? "(no broker stated)" : raw, rowIds: [] };
  ...
  const visibleRows = useMemo(() => {
    ...
    return derived.filter((item) => {
      const haystack = [item.row.raw_security_text, item.row.raw_isin, item.row.raw_broker_text, item.row.raw_source_reference]
        .filter(Boolean)
        .join(" ")
        .toUpperCase();
        ...
        await Promise.all(
          chunk.map(async (item) => {
            ...
            const base = {
              ...
              candidate_fingerprint: fingerprint([choice.securityId, txnType, tradeDate, quantity, row.raw_source_reference]),
            };

            const readyQuality = {
              data_quality_state: verdict.issues.length === 0 ? "VALID" : "INCOMPLETE",
            ...
            const payload = choice.excluded
              ? {
                  ...base,
                  resolution: "EXCLUDED",
                  data_quality_state: verdict.ready ? readyQuality.data_quality_state : verdict.dataQualityState,
                  data_quality_issues: verdict.ready ? readyQuality.data_quality_issues : verdict.issues,
                }
              : verdict.ready
                ? { ...base, resolution: "RESOLVED", ...readyQuality }
                : {
                    ...base,
            ...
            const { error } = await supabase.from("import_source_rows").update(payload).eq("id", row.id);
  ...
  const commit = useMutation({
    ...
    onSuccess: async (result) => {
      toast.success(`${result.committed_transaction_count} transactions committed · ${result.excluded_row_count} rows excluded`);
  ...
  function applyBrokerMapping(groupKey: string, accountId: string, force = false) {
    ...
    setChoices((prev) => {
      ...
      for (const rowId of group.rowIds) {
        ...
        if (!force && current?.brokerAccountId) { preserved += 1; continue; }
    ...
    toast.success(`${changed} rows from ${group.label} mapped${preserved > 0 ? ` · ${preserved} existing row choices preserved` : ""}`);
  }
  ...
  return (
    <>
      <PageHeader
        ...
        description={`${batch.total_source_rows ?? rows.length} source rows · state ${batch.state}. Transactions remain the source of truth; unknown dates or genuinely unstated brokers stay explicitly incomplete rather than being guessed.`}
        actions={<Button asChild size="sm" variant="ghost"><Link to="/import">All batches</Link></Button>}
      />

      {isCommitted ? (
        <>
          <div className="mb-4 rounded-md border border-emerald-500/30 bg-emerald-500/5 px-4 py-3 text-sm text-emerald-400">
            This batch is committed. Its source interpretation is frozen; missing broker accounts can be filled later through the trusted repair workflow.
          </div>
      ...
      <section className="mb-4 grid gap-3 rounded-lg border border-border bg-card p-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Stat label="Incomplete but usable" value={incompleteReadyCount} />
      </section>
      ...
      {!isCommitted ? (
        <section className="mb-4 flex flex-wrap items-end gap-3 rounded-lg border border-border bg-card p-4">
          <div className="w-40 space-y-1.5">
            ...
            <Input id="currency" maxLength={3} placeholder="INR" value={declaredCurrency} onChange={(e) => setDeclaredCurrency(e.target.value.toUpperCase())} />
          </div>
          <Button variant="outline" onClick={confirmAllSuggestions}>Confirm all single suggestions</Button>
          <Button variant="outline" onClick={excludeBlocked} disabled={blockedCount === 0}>Exclude {blockedCount} blocked rows</Button>
          <Button variant="ghost" onClick={() => setShowAccounts((v) => !v)}>{showAccounts ? "Hide" : "Add"} broker account</Button>
          <div className="ml-auto flex gap-2">
            <Button variant="outline" onClick={() => save.mutate()} disabled={save.isPending}>{save.isPending ? "Saving…" : "Save review"}</Button>
            <Button onClick={() => commit.mutate()} disabled={commit.isPending || readyCount === 0 || blockedCount > 0}>
              {commit.isPending ? "Committing…" : `Commit ${readyCount} rows`}
            </Button>
          </div>
          {blockedCount > 0 ? (
            <p className="w-full text-xs text-muted-foreground">Resolve or exclude the {blockedCount} genuinely blocked rows. Unknown date and genuinely unstated broker/account are no longer blockers when every other required fact is valid.</p>
          ) : (
            <p className="w-full text-xs text-muted-foreground">All included rows are commit-eligible. Rows with unknown date or broker remain visibly INCOMPLETE and are never guessed.</p>
          )}
        </section>
      ) : null}
      ...
      {!isCommitted ? (
        <section className="mb-4 rounded-lg border border-border bg-card p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Map each stated source broker only to its matching demat account. Blank source broker rows are deliberately left unknown unless you explicitly identify them.</p>
            ...
            {namedBrokerGroups.map((group) => {
              ...
              return (
                <div key={group.key} className="flex flex-wrap items-center gap-3">
                  ...
                  <span className="w-28 font-mono text-[11px] text-muted-foreground">{assigned}/{group.rowIds.length} set</span>
                  <Select value={brokerMap[group.key] ?? ""} onValueChange={(value) => { setBrokerMap((prev) => ({ ...prev, [group.key]: value })); applyBrokerMapping(group.key, value); }}>
                    <SelectTrigger className="h-8 w-[240px]"><SelectValue placeholder="Choose account" /></SelectTrigger>
                  ...
                  <Button size="sm" variant="ghost" disabled={!brokerMap[group.key]} onClick={() => {
                    const accountId = brokerMap[group.key];
                    if (accountId && window.confirm(`Replace the account on all ${group.rowIds.length} rows whose source broker is ${group.label}?`)) applyBrokerMapping(group.key, accountId, true);
                  }}>Re-apply to all</Button>
                </div>
              );
            })}
          </div>
          {blankBrokerGroup ? (
            <div className="mt-4 rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-xs text-muted-foreground">
              ...
              M11 allows these rows to enter the ledger with broker account left NULL and quality flags MISSING_BROKER + MISSING_ACCOUNT. Their quantities still contribute to Holdings. You may identify and fill the broker later.
            </div>
          ) : null}
        </section>
      ) : null}

      {showAccounts ? <div className="mb-4"><BrokerAccountsPanel compact /></div> : null}
        ...
        <section className="mb-4 rounded-lg border border-border bg-card p-4">
          <div className="flex flex-wrap items-end gap-3">
            <div className="min-w-[210px] flex-1 space-y-1.5">
              ...
              <Input id="row-search" placeholder="Ticker, ISIN, broker or reference" value={rowSearch} onChange={(e) => setRowSearch(e.target.value)} />
            </div>
            ...
            <Button variant="outline" onClick={selectAllVisible} disabled={selectableVisibleIds.length === 0}>Select visible ({selectableVisibleIds.length})</Button>
            <Button variant="ghost" onClick={() => setSelectedRows({})} disabled={selectedCount === 0}>Clear selection</Button>
          </div>
          <div className="mt-3 flex flex-wrap items-end gap-3 rounded-md border border-border bg-background/30 p-3">
            ...
            <Button onClick={assignSelectedRows} disabled={selectedCount === 0 || !bulkAccountId}>Assign {selectedCount} selected</Button>
            <p className="max-w-2xl text-xs text-muted-foreground">Optional. Leave broker-unknown rows untouched if you do not know the account. Selection is transaction-level, never ticker-level.</p>
          </div>
        </section>
      ) : null}
        ...
        <table className="w-full min-w-[1180px] text-sm">
          <thead className="bg-muted/40 text-left font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
            <tr>
              ...
              <th className="px-3 py-2.5">#</th><th className="px-3 py-2.5">Source</th><th className="px-3 py-2.5">Interpreted</th><th className="px-3 py-2.5">Security</th><th className="px-3 py-2.5">Account</th><th className="px-3 py-2.5">Status</th><th className="px-3 py-2.5" />
            </tr>
          </thead>
            ...
            {visibleRows.map((item) => {
              ...
              return (
                <tr key={row.id} className={choice.excluded ? "opacity-50" : undefined}>
                  {!isCommitted ? <td className="px-3 py-3 align-top"><input type="checkbox" aria-label={`Select source row ${row.source_row_number}`} disabled={!canSelect} checked={Boolean(selectedRows[row.id])} onChange={(e) => setSelectedRows((prev) => { const next = { ...prev }; if (e.target.checked) next[row.id] = true; else delete next[row.id]; return next; })} /></td> : null}
                  <td className="px-3 py-3 font-mono text-xs text-muted-foreground">{row.source_row_number}</td>
                  <td className="max-w-[220px] px-3 py-3"><p className="truncate text-xs text-foreground">{row.raw_security_text ?? "—"}</p><p className="truncate font-mono text-[11px] text-muted-foreground">{row.raw_isin ?? "no ISIN"} · {row.raw_broker_text?.trim() || "broker not stated"}</p></td>
                  <td className="px-3 py-3 font-mono text-[11px] text-muted-foreground"><div>{txnType ?? "type?"}</div><div>{tradeDate ?? "date?"}</div><div>{quantity ?? "qty?"}</div></td>
                  <td className="px-3 py-3">
                    {isCommitted ? <span className="font-mono text-[11px]">{chosen?.name ?? "—"}</span> : candidates && candidates.securities.length > 0 ? (
                      <div className="space-y-1">
                        <Select value={choice.securityId ?? ""} onValueChange={(value) => setChoice(row.id, { securityId: value })}>
                          ...
                          <SelectContent>{candidates.securities.map((security: Security) => <SelectItem key={security.id} value={security.id}>{security.name} · {security.exchange ?? "—"}:{security.primary_symbol ?? "—"}</SelectItem>)}</SelectContent>
                        </Select>
                        <StatusBadge tone={choice.securityId ? "ok" : "info"}>{choice.securityId ? "confirmed" : `suggested · ${candidates.basis ?? "match"}`}</StatusBadge>
                      </div>
                    ) : <StatusBadge tone="warn">no master match</StatusBadge>}
                  </td>
                  <td className="px-3 py-3">
                    {isCommitted ? <span className="font-mono text-[11px]">{accounts.find((a) => a.id === choice.brokerAccountId)?.nickname ?? "unknown"}</span> : (
                      <Select value={choice.brokerAccountId ?? ""} onValueChange={(value) => setChoice(row.id, { brokerAccountId: value })}>
                        <SelectTrigger className="h-8 w-[200px]"><SelectValue placeholder={row.raw_broker_text?.trim() ? "Choose account" : "Unknown allowed"} /></SelectTrigger>
                        <SelectContent>{accounts.map((account) => <SelectItem key={account.id} value={account.id}>{account.nickname} · {brokerName(account.broker_id)}</SelectItem>)}</SelectContent>
                      </Select>
                    )}
                  </td>
                  <td className="px-3 py-3">
                    {choice.excluded ? <StatusBadge tone="neutral">excluded</StatusBadge> : verdict.ready ? (
                      <div className="space-y-1">
                        <StatusBadge tone="ok">ready</StatusBadge>
                        {verdict.issues.includes("MISSING_BROKER") ? <StatusBadge tone="warn">broker unknown · resolve later</StatusBadge> : null}
                        {verdict.issues.includes("MISSING_DATE") ? <StatusBadge tone="warn">date unknown · kept empty</StatusBadge> : null}
                      </div>
                    ) : (
                      <div className="space-y-1"><IssueBadges issues={verdict.issues} /><p className="max-w-[240px] text-[11px] text-muted-foreground">{verdict.blockingReasons[0]}</p></div>
                    )}
                    {isUnsupportedTxnType(txnType) ? <StatusBadge tone="bad" className="mt-1">corporate action not interpreted</StatusBadge> : null}
                  </td>
                  <td className="px-3 py-3 text-right">
                    {!isCommitted && row.resolution !== "COMMITTED" ? <div className="flex justify-end gap-1"><Button size="sm" variant="ghost" onClick={() => setChoice(row.id, { excluded: !choice.excluded })}>{choice.excluded ? "Include" : "Exclude"}</Button>{batch.source_format === "MANUAL" ? <Button size="sm" variant="ghost" className="text-destructive" disabled={removeRow.isPending} onClick={() => removeRow.mutate(row.id)}>Delete</Button> : null}</div> : null}
                  </td>
                </tr>
              );
            })}
      ...
      {visibleRows.length === 0 ? <p className="mt-3 text-sm text-muted-foreground">No transactions match the current filter.</p> : null}
    </>
  );
}

function Stat({ label, value }: { label: string; value: number }) {
  return <div><p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{label}</p><p className="mt-1 font-mono text-xl text-foreground">{value}</p></div>;
}
...
function Reconciliation({ batchId, portfolioId }: { batchId: string; portfolioId: string }) {
  ...
  const holdings = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("current_holdings").select("*").eq("portfolio_id", portfolioId);
      if (error) throw new Error(error.message);
      ...
      const inputs = (claims ?? []).map((claim) => ({ isin: null, exchange: null, securityText: claim.ticker }));
      const index = await buildMasterIndex(supabase, inputs);
      return (claims ?? []).map((claim, i) => {
        ...
        return { ticker: claim.ticker, netUnits: claim.netUnits, security: unique, ambiguous: candidates.securities.length > 1, derived: unique ? (bySecurity.get(unique.id) ?? null) : null };
      });
    },
  });
  ...
  return (
    <section className="mb-4 rounded-lg border border-border bg-card p-5">
      ...
      <p className="mt-1 text-xs text-muted-foreground">HOLDINGS is reconciliation evidence only. Transactions remain the accounting source of truth, compared by canonical security identity rather than raw ticker text.</p>
      ...
      {holdings.data ? (
        <div className="mt-3 max-h-80 overflow-auto rounded border border-border">
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-muted/60 text-left font-mono uppercase tracking-wider text-muted-foreground"><tr><th className="px-3 py-2">Ticker</th><th className="px-3 py-2">Matched security</th><th className="px-3 py-2 text-right">Sheet units</th><th className="px-3 py-2 text-right">Derived units</th><th className="px-3 py-2">Status</th></tr></thead>
            <tbody className="divide-y divide-border">
              {holdings.data.map((entry) => {
                ...
                return <tr key={entry.ticker}><td className="px-3 py-1.5 font-mono">{entry.ticker}</td><td className="px-3 py-1.5">{entry.security ? <span className="font-mono text-[11px]">{entry.security.name}</span> : entry.ambiguous ? <StatusBadge tone="warn">ambiguous ticker</StatusBadge> : <StatusBadge tone="neutral">no unique match</StatusBadge>}</td><td className="px-3 py-1.5 text-right font-mono">{entry.netUnits ?? "—"}</td><td className="px-3 py-1.5 text-right font-mono">{derivedValue ?? "UNAVAILABLE"}</td><td className="px-3 py-1.5">{!entry.security ? <StatusBadge tone="neutral">not compared</StatusBadge> : derivedValue === null ? <StatusBadge tone="neutral">not derived</StatusBadge> : same ? <StatusBadge tone="ok">matches</StatusBadge> : <StatusBadge tone="bad">differs</StatusBadge>}</td></tr>;
              })}
            </tbody>
          </table>
        </div>
