...
-- PortfolioAI Migration 06 — untrusted import staging and provenance layer.
--
-- Invariants:
--   * Staging rows are NOT canonical ledger facts. public.transactions remains
--     the accounting source of truth; only a later trusted server-side commit
--     path may create canonical rows from approved staging rows.
--   * Raw source evidence is immutable; interpretation is mutable until commit,
--     then frozen for auditability.
--   * Missing stays missing: a failed parse leaves the candidate NULL, never 0.
--   * No fabricated securities, broker accounts, dates or prices.
--   * MISSING_BROKER (unknown institution) and MISSING_ACCOUNT (institution
--     known, account not identified) remain semantically distinct.
--   * No economic-content uniqueness; duplicates are flagged, never discarded.
--   * All foreign keys are ON DELETE RESTRICT. No CASCADE anywhere.
--   * Lineage on transactions is import_source_row_id ONLY; batch is derivable.
--   * Reuses public.set_updated_at(); it is NOT redeclared.
--   * No commit_import_batch RPC here. No service-role application credential;
--     the service_role GRANT is Supabase/admin compatibility only.
--
-- Rollback (safe only while no canonical transaction carries lineage; the
-- RESTRICT FK blocks the drop otherwise, which is the intended protection):
-- begin;
...
-- -- then recreate public.transactions_guard_immutable_fields() from its
-- -- Migration 05 definition (without the lineage check).
...
-- commit;
-- (never `cascade`; never drop the shared public.set_updated_at())
...
-- Extend the Migration 05 guard to lineage (behaviour otherwise identical).
